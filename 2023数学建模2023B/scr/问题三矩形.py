...
def adjust_position_for_overlap(prev_position, prev_coverage_width, initial_estimate_position):
    ...
    # 微调的量
    adjustment = METERS_IN_ONE_NAUTICAL_MILE * 5  # 1% of a nautical mile as initial adjustment

    # 如果重叠率不在10%~20%范围内，进行调整
    while not (10 <= overlap_rate <= 15):
        if overlap_rate < 10:
            current_position -= adjustment  # 减少位置以增加重叠
            print(f"Adjustment: {adjustment:.2f}")
            print(f"Updated Position: {current_position:.2f}")
            print(f"Updated Overlap Rate: {overlap_rate:.2f}%")
    ...
    print(f"Initial Overlap Rate: {overlap_rate:.2f}%")
    print(f"Initial Position: {current_position:.2f}")
...
def calculate_survey_lines(total_width):
    initial_depth = calculate_depth(0, 0)
    initial_coverage_width = calculate_coverage_width(initial_depth)

    # 步骤1: 调整起始位置
    lines_positions = [0.5 * initial_coverage_width]
    overlap_rates = []
    ...
    # 步骤2: 调整结束条件
    while current_position < total_width + 0.5 * current_coverage_width:
